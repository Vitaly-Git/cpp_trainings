...
1. Мы используем, хотя и модифицированный, но двоичный поиск, где выполняется 
   рекурсивный вызов функции для обработки N элементом массива. Вызов функции 
   выполняется log(N) раз, при этом рекурсивный вызов функции постепенно 
   увеличивает стек, и размер стека будет пропорционален количеству вызовов, 
   т.е. log(N). Таким образом пространственная сложность будет равна O(log(N)).
*/

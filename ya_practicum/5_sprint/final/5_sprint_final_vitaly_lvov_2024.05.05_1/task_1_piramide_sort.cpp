...
2. Скорость вставки в кучу выполняется за O(log(N)).
3. Скорость удаления из кучи выполняется за O(log(N)).
4. Обе операции (2 и 3) мы используем для пирамидальной сортировки для N элементов.
5. Таким образом максимальная сложность пирамидальной сортировки: O(Nlog(N))
-- ПРОСТРАНСТВЕННАЯ СЛОЖНОСТЬ --
1. Куча в памяти представляется как вектор размером N, где N-количество элементов для сортировки. 
   Таким образом, сложность этого этапа O(N). 
// Спасибо, точно, простите, упустил рекурсию.
2. Мы используем рекурсивный вызов функций при просеивании вверх и вниз. 
   Рекурсивные вызовы будут выполняться O(log(N)) раз для каждого элемента, увеличивая размер стека.
   Данное увеличение будет повторяться для каждого элемента, увеличвая стек от 0 до O(log(N)), при
   завершении рекурсии освобождая его.
   Таким образом пространственная сложность этого этапа будет равна O(log(N)).
3. Итоговая пространственная сложность решения равна O(N)+O(log(N)). 
*/

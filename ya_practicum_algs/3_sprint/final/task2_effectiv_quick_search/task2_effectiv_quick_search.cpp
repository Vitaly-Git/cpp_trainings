...
1. Сложность быстрой сортировки во многом зависит от выбора опорного элемента
   и варьируется от O(n^2) до O(nlog(n)). Существуют методы ускорения быстрой
...
1. В данном случаем мы используем сортировку перестановкой, т.е. без создания дополнительных
   массивов. Соответственно, нам может показаться, что дополнительных ресурсов по памяти 
   не расходуется, но стоит учитывать, что мы вызываем функцию рекурсивно, т.е. потребляем 
   ресурсы стека. Расход ресурсов стека пропорционален количеству вызовов, т.е. от O(n^2) 
   в худшем случае, до O(nlog(n) раз в лучшем. Таким образом, пространсвенная сложность 
   в худшем случае равна O(n^2).
*/
